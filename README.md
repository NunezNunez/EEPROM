...
void cmdout(int cmd){
  // Send the 10-bit EEPROM instruction
  dataOutput(cmd, 10);
}


int dataInput(byte size){
  // Read bits from EEPROM DO pin
  int data = 0;
...
void dataOutput(int data, byte cnt){
  // Send data MSB first to EEPROM DI pin
...
void EEPROM_wral(byte data){
  ...
  // Send 8-bit value to write everywhere
  dataOutput(data, 8);

  digitalWrite(CS, LOW);

  // EEPROM internal programming cycle
  delay(10);
}
...
int EEPROM_read(byte adr){
  ...
  // AT93C46 sends one dummy 0 before the actual data
  dataInput(1);
...
void EEPROM_write(byte adr, int data){
  ...
  // Send the 8-bit value
  dataOutput(data, 8);

  digitalWrite(CS, LOW);

  // Maximum write cycle is 5 ms;
  // 10 ms is a conservative wait
  delay(10);
}
